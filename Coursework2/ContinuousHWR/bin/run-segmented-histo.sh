...
# Run histogram segmenting system
...
do
  ...
  do
    ...
    TIMEARR[i]=$( { time ./run-recog.sh $CLASS points segmentation-recog.sh .ss_refs $WFILE HistogramSegmentation hard mean scale > .ss-mod${i}.tmp; } 2>&1 )
