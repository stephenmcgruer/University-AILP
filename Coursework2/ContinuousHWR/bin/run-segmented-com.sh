...
# Run stroke segmenting system with centre of mass
...
do
  ...
  do
    ...
    TIMEARR[i]=$( { time ./run-recog.sh $CLASS points segmentation-recog.sh .ss_refs $WFILE StrokeMeanSegmentation hard mean scale > .ss-mod${i}.tmp; } 2>&1 )
    ./chwr-ana.pl .ss-mod${i}.tmp > .ss-mod${i}.out

    ACCARR[i]=`grep "WORD" .ss-mod${i}.out | cut -d " " -f 4 | cut -d "=" -f 2`
