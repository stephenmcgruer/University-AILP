...
# Run the 'best' system - mean and scale normalisation with
# preselection multitemplating.
...
do
  ...
  do
    ...
    TIMEARR[i]=$( { time ./run-dtw-dist-mt.sh $BESTNOTMOD $MOD $CLASS mean scale | ./mt-knn.pl > .mod${i}.tmp; } 2>&1 )
